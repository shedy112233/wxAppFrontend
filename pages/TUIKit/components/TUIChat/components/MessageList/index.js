...
const imgMsg = {
  ...
  protocol: "JSON",
  random: null,
  readReceiptInfo: {readCount: undefined, unreadCount: undefined, isPeerRead: undefined},
  revoker: null,
  senderTinyID: "",
  sequence: null,
  status: "success",
...
Component({
  ...
  methods: {
    // 刷新消息列表
    refresh() {
      console.log('--------this.data.isCompleted-------',this.data.isCompleted)
      if (this.data.isCompleted) {
        this.setData({
          isCompleted: true,
          triggered: false,
        });
        console.log('this.data.messageList=======',this.data.messageList[0])
        if(this.data.messageList[0].conversationType != 'GROUP'){
          wx.request({
            url: app.globalData.baseUrl + '/wx/getPersonMsg',
            data:{
              userId: app.globalData.config.userID,
              toUserId: this.data.conversation.conversationID.slice(3),
              time: this.data.messageList[0].time
            },
            method: 'POST',
            ...
            success: res => {
              ...
              if(result.status === 200) {
                ...
                const msgVos = info.msgList
                console.log(msgVos)
                msgVos.forEach(item => {
                  if(item.MsgBody[0].MsgType === 'TIMTextElem'){
                    const textMsg = {}
                    textMsg.ID=item.MsgKey
                    textMsg.atUserList=[]
                    textMsg.avatar=""
                    textMsg.clientSequence=item.MsgSeq
                    textMsg.clientTime=item.MsgTimeStamp
                    textMsg.cloudCustomData="{\"messageFeature\"={\"needTyping\"=1\"version\"=1}}"
                    textMsg. conversationID=this.data.conversation.conversationID
                    textMsg.conversationSubType=undefined
                    textMsg.conversationType="C2C"
                    if(app.globalData.config.userID === item.From_Account){
                      textMsg.flow="out"
                    } else {
                      textMsg.flow="in"
                    }
                    textMsg.from=item.From_Account
                    textMsg.isBroadcastMessage=false
                    textMsg.isDeleted=false
                    textMsg.isModified=false
                    textMsg.isPeerRead=true
                    textMsg.isPlaceMessage=0
                    textMsg.isRead=true
                    textMsg.isResend=false
                    textMsg.isRevoked=false
                    textMsg.isShowMoreHistoryTime=true
                    textMsg.isSupportExtension=false
                    textMsg.isSystemMessage=false
                    textMsg.nameCard=""
                    textMsg.needReadReceipt=false
                    textMsg.nick=""
                    textMsg.payload={text:item.MsgBody[0].MsgContent.Text}
                    textMsg.priority="Normal"
                    textMsg.protocol="JSON"
                    textMsg.random=item.MsgRandom
                    textMsg.readReceiptInfo={readCount:undefined,unreadCount:undefined,isPeerRead:undefined}
                    textMsg.revoker=null
                    textMsg.senderTinyID=""
                    textMsg.sequence=item.MsgSeq
                    textMsg.status="success"
                    textMsg.time=item.MsgTimeStamp
                    textMsg.to=item.To_Account
                    textMsg.type="TIMTextElem"
                    textMsg.version=0
                    if(list.length > 0) {
                      const flag = list.some(obj => {
                        return obj.sequence == textMsg.sequence
                      })
                      console.log('text类型聊天记录-----------------',flag)
                      if(!flag) {
                        list.push(textMsg)
                      }
                    } else {
                  ...
                  if(item.MsgBody[0].MsgType === 'TIMImageElem') {
                    imgMsg.ID = item.MsgKey
                    imgMsg.clientSequence = item.MsgSeq
                    imgMsg.clientTime = item.MsgTimeStamp
                    imgMsg.conversationID = this.data.conversation.conversationID
                    imgMsg.from = item.From_Account
                    if(app.globalData.config.userID === item.From_Account){
                      imgMsg.flow="out"
                    } else {
                      imgMsg.flow="in"
                    }
                    const imageParam = item.MsgBody[0].MsgContent.ImageInfoArray
                    imageParam.forEach(item => {
                      switch(item.sizeType){
                        case 1:
                          item.imageUrl = item.imageUrl + '?imageMogr2/'
                          item.url = item.imageUrl + '?imageMogr2/'
                          break;
                        case 2:
                          item.imageUrl = item.imageUrl + '?imageMogr2/&imageView2/3/w/720/h/720'
                          item.url = item.imageUrl + '?imageMogr2/&imageView2/3/w/720/h/720'
                          break;
                        case 3:
                          item.imageUrl = item.imageUrl + '?imageMogr2/&imageView2/3/w/198/h/198'
                          item.url = item.imageUrl + '?imageMogr2/&imageView2/3/w/198/h/198'
                          break;
                      }
                    })
                    item.MsgBody[0].MsgContent.imageInfoArray = imageParam
                    imgMsg.payload = item.MsgBody[0].MsgContent
                    imgMsg.random = item.MsgRandom
                    imgMsg.sequence = item.MsgSeq
                    imgMsg.time = item.MsgTimeStamp
                    imgMsg.to = item.To_Account
                    if(list.length > 0) {
                      const flag = list.some(obj => {
                        return obj.sequence = imgMsg.sequence && obj.type === imgMsg.type
                      })
                      if(!flag) {
                        list.push(imgMsg)
                      }
                ...
                this.showMoreHistoryMessageTime(list);
                this.showHistoryMessageTime(list);
                this.data.messageList = [...list, ...this.data.messageList];
                // if (list.length > 0 && this.data.messageList.length < this.data.unreadCount) {
                //   this.getMessageList(conversation);
                // }
                this.$handleMessageRender(this.data.messageList, list);
                this.filterRepateSystemMessage(this.data.messageList)
              }
            }
          })
        }
      ...
      if (!this.data.isCompleted) {
        wx.$TUIKit.getMessageList({
          ...
          if(res.data.messageList.length > 0){
            if(res.data.messageList[0].conversationType === 'C2C'){ // 更新单聊会话状态
              wx.request({
                url: app.globalData.baseUrl + '/im/updateConversation',
                data:{
                  conversationId: res.data.messageList[0].conversationID
                },
                method: 'POST',
                header: { 'content-type': 'application/x-www-form-urlencoded'},
                success: res => {
            ...
            if(res.data.messageList[0].conversationType === 'GROUP'){ // 更新群聊会话状态
              wx.request({
                url: app.globalData.baseUrl + '/im/updateGroupInfo',
                data:{
                  conversationId: res.data.messageList[0].conversationID
                },
                method: 'POST',
    ...
    showHistoryMessageTime(messageList) {
      ...
      for (let index = 0; index < messageList.length; index++) {
        ...
        if (nowadayTime - firstTime > cut) {
          ...
          console.log('--------------messageList['+ indexbutton + ']----------')
          console.log(messageList[indexbutton])
          console.log('--------------messageList['+ indexbutton + ']----------')
          this.setData({
