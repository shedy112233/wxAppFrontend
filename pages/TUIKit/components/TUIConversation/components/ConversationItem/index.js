  ...
  properties: {
    conversation: {
      ...
      observer(conversation) {
        ...
        const phone = conversation.userProfile.userID
        wx.request({
          url: app.globalData.baseUrl + '/wx/getUserBz',
          data:{
          ...
          success: res => {
            ...
            if(result.status === 200){
              console.log(result.data.bz)
              conversation.userProfile.bz = result.data.bz
              console.log( conversation.userProfile.bz)
            } else {
              conversation.userProfile.bz = ''
            }
            this.setData({
              conversationName: this.getConversationName(conversation),
              setConversationAvatar: this.setConversationAvatar(conversation),
  ...
  methods: {
    ...
    getConversationName(conversation) {
      ...
      if (conversation.type === 'C2C') {
        return conversation.remark || conversation.userProfile.bz || conversation.userProfile.nick || conversation.userProfile.userID;
      }
      if (conversation.type === 'GROUP') {
        ...
        success: (res) => {
          if (res.confirm) {
            ...
            if(this.data.conversation.type === 'C2C') {
              ...
              wx.request({
                url: app.globalData.baseUrl + '/im/delConversation',
                data:{
            ...
            if(this.data.conversation.type === 'GROUP'){
              wx.request({
                url: app.globalData.baseUrl + '/im/delGroupInfo',
                data:{
                  conversationId: this.data.conversation.conversationID
                },
                method: 'POST',
